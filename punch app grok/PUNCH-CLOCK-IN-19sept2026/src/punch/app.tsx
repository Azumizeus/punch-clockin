...
export function PunchApp() {
  ...
  useEffect(() => {
    if (!connected) return;
    const id = window.setInterval(() => pushFeed(), 7000);
  ...
  return (
    <div className="min-h-dvh bg-bg text-fg">
      ...
      <div className="mx-auto flex min-h-dvh w-full max-w-md flex-col">
        {!connected ? <ConnectScreen /> : <SignedIn />}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative flex min-h-dvh flex-col bg-bg">
      ...
      <main className={cn("flex-1", showNav && "pb-24")}>
        ...
        {view === "app" && tab === "punch" ? <PunchScreen /> : null}
      ...
      {showNav ? (
        <nav className="fixed bottom-0 left-1/2 z-10 w-full max-w-md -translate-x-1/2 border-t border-line bg-bg pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1">
